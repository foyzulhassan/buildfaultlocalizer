...
public class FullLogFaultLocalization {
	...
	public void simAnalyzerFullLog() throws Exception {
		...
		for (int index = 0; index < projects.size(); index++) {
			...
			projects.get(index).setEvFulllogPos(topn);
			projects.get(index).setEvFulllogMrr(mrr);
			projects.get(index).setEvFulllogMap(map);
		}
