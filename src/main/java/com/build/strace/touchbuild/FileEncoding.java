...
public enum FileEncoding {
	 ...
	 * complicated than it initially sounds. However, with a liberal sprinkling
	 * of common sense we can arrive at a reasonable implementation that will
	 * work nine times out of ten.
	 * 
	 * For instances in which this software fails to produce a desirable result
	 * - we can simply hardcode in patterns that will override its logic and
	 * deliver the specified answer when pattern matches are encountered.
	 * 
	 * @param file
	 *            the file whose content we are about to test (are the file
	 *            contents plain text or not)?
	 * @param acceptUTF8encoding
	 *            if <ff>true</ff>, a file in UTF-8 encoding will be accepted if
	 *            it contains only characters existing in the Latin-1 table.
	 * @return true if the file contents are textual in nature, false otherwise.
	 * @throws IOException
	 *             if problems reading the file are encountered.
	 */
	public boolean contentIsText(final File file, boolean acceptUTF8encoding) throws IOException {
		...
		try {
			...
			try {
				...
				System.out.println("Failure closing the file input stream or logging the file absolute path." + "\n"
						+ th.toString() + "\n");
			}
		}
		return isText;
